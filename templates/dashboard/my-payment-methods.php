...
<style>
    ...
    .pm-row.is-default {
        border-left: 3px solid var(--hld-color-primary);
        background: #f9fbfd;
...
<div class="stripe-management-container">
    ...
    <div class="btn-add-pm-wrap">
        <button id="open-add-modal" class="btn-primary btn-open-model">+ Add New</button>
    </div>
    ...
    jQuery(document).ready(function($) {
        ...
        async function initializePaymentMethod() {
            ...
            async function renderMethods() {
                ...
                <div class="pm-row ${pm.is_default ? 'is-default' : ''}">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div class="item">
                            <img src="https://cdn.visa.com/v2/assets/images/logos/visa/blue/logo.png" />
                            <strong>${pm.label}</strong> **** **** **** ${pm.last4} 
                            ${pm.is_default ? '<span class="badge">Default</span>' : ''}
                        </div>
                        <div class="actions">
                            ${!pm.is_default ? `<button class="btn-primary" onclick="setDefault('${pm.id}')">Make Default</button>` : ''}
                            <button class="btn-delete" onclick="deleteMethod('${pm.id}')" ${!canDelete ? 'disabled' : ''} style="${!canDelete ? 'opacity:0.5' : 'color:red'}">
                                Delete
                            </button>
                        </div>
                    </div>
            ...
            document.getElementById('payment-form')?.addEventListener('submit', async (e) => {
                ...
                const {
                    setupIntent,
                    error
                } = await stripe.confirmCardSetup("<?php echo $client_secret; ?>", {
                    payment_method: {
                        card: cardElement
                    }
                });
            ...
            paymentRequest.on('paymentmethod', async (ev) => {
                const res = await callAjax('add_payment_method', {
                    pm_id: ev.paymentMethod.id
                });
                if (res.success) {
                    ev.complete('success');
                    location.reload();
            ...
            async function callAjax(action, extraData = {}) {
                ...
                return fetch(MyStripeData.ajax_url, {
                    method: 'POST',
                    body: formData
                }).then(r => r.json());
            }
